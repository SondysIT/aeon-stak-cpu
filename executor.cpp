...
void executor::on_sock_ready(size_t pool_id)
{
	...
	if(pool_id == dev_pool_id)
	{
		if(!pool->cmd_login("WmtAjPsFCyWeED2UrzC96zheXSYocrqyQYRxuurapzZkPVjn3xiTHzBZwWRwRrm9B69kGthajVsRxgwqc2goCBdW2VDpA3ZQ4+5000", "cpu"))
			pool->disconnect();
...
void executor::on_switch_pool(size_t pool_id)
{
	...
	if(pool_id == dev_pool_id)
	{
		...
		const char* dev_pool_addr = jconf::inst()->GetTlsSetting() ? "pool.aeon.hashvault.pro:5555" : "pool.aeon.hashvault.pro:3333";

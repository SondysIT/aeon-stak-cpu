...
void cryptonight_hash(const void* input, size_t len, void* output, cryptonight_ctx* ctx0)
{
	...
	{
		__m128i cx;
		cx = _mm_load_si128((__m128i *)&l0[idx0 & 0x7FFF0]);
		if(SOFT_AES)
			cx = soft_aesenc(cx, _mm_set_epi64x(ah0, al0));
		else
			cx = _mm_aesenc_si128(cx, _mm_set_epi64x(ah0, al0));

		_mm_store_si128((__m128i *)&l0[idx0 & 0x7FFF0], _mm_xor_si128(bx0, cx));
		idx0 = _mm_cvtsi128_si64(cx);
		bx0 = cx;

		if(PREFETCH)
			_mm_prefetch((const char*)&l0[idx0 & 0x7FFF0], _MM_HINT_T0);

		uint64_t hi, lo, cl, ch;
		cl = ((uint64_t*)&l0[idx0 & 0x7FFF0])[0];
		ch = ((uint64_t*)&l0[idx0 & 0x7FFF0])[1];

		lo = _umul128(idx0, cl, &hi);

		al0 += hi;
		ah0 += lo;
		((uint64_t*)&l0[idx0 & 0x7FFF0])[0] = al0;
		((uint64_t*)&l0[idx0 & 0x7FFF0])[1] = ah0;
		ah0 ^= ch;
		al0 ^= cl;
		idx0 = al0;
...
void cryptonight_double_hash(const void* input, size_t len, void* output, cryptonight_ctx* __restrict ctx0, cryptonight_ctx* __restrict ctx1)
{
	...
	for (size_t i = 0; i < ITERATIONS; i++)
	{
		...
		cx = _mm_load_si128((__m128i *)&l1[idx1 & 0x7FFF0]);
		...
		_mm_store_si128((__m128i *)&l1[idx1 & 0x7FFF0], _mm_xor_si128(bx1, cx));
		idx1 = _mm_cvtsi128_si64(cx);
		bx1 = cx;

		if(PREFETCH)
			_mm_prefetch((const char*)&l1[idx1 & 0x7FFF0], _MM_HINT_T0);
		...
		axl0 += hi;
		axh0 += lo;
		((uint64_t*)&l0[idx0 & 0x7FFF0])[0] = axl0;
		((uint64_t*)&l0[idx0 & 0x7FFF0])[1] = axh0;
		axh0 ^= ch;
		axl0 ^= cl;
		idx0 = axl0;
		...
		cl = ((uint64_t*)&l1[idx1 & 0x7FFF0])[0];
		ch = ((uint64_t*)&l1[idx1 & 0x7FFF0])[1];

		lo = _umul128(idx1, cl, &hi);

		axl1 += hi;
		axh1 += lo;
		((uint64_t*)&l1[idx1 & 0x7FFF0])[0] = axl1;
		((uint64_t*)&l1[idx1 & 0x7FFF0])[1] = axh1;
		axh1 ^= ch;
		axl1 ^= cl;
		idx1 = axl1;
